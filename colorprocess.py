...
def process_images():
    preprocessed_folder = Path("path/to/your/preprocessed_folder")
    output_folder = preprocessed_folder / "output_images"
    output_folder.mkdir(parents=True, exist_ok=True)

    for i in range(10, 50):
        print(f"Processing image {i}")
        file_path = preprocessed_folder / f"{i}.png"
        ...
        if img is not None:
            ...
            hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)  # Make sure to initialize hsv before using it
            ...
            for j, color_range in enumerate(color_ranges):
                ...
                output_path = output_folder / f"cannabis-{i}-{colors[j]}-Segmented.png"
                cv2.imwrite(str(output_path), segmented)

            output_path = output_folder / f"cannabis-{i}-Contour.png"
